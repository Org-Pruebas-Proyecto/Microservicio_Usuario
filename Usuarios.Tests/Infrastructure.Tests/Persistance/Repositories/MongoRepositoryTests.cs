...
public class MongoRepositoryTests
{
    ...
    [Fact]
    public async Task GetByIdAsync_DeberiaRetornarDocumento_CuandoExiste()
    {
        // Arrange
        var documentoId = Guid.NewGuid().ToString();
        var actividadEsperada = new ActividadMongo
        {
            Id = Guid.Parse(documentoId),
            UsuarioId = Guid.NewGuid(),
            TipoAccion = "LOGIN",
            Detalles = "Inicio de sesión",
            Fecha = DateTime.UtcNow
        ...
        asyncCursor.SetupSequence(x => x.MoveNextAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(true)
            .ReturnsAsync(false);
        asyncCursor.Setup(x => x.Current).Returns(new[] { actividadEsperada });

        mockCollection.Setup(c => c.FindAsync(
                It.Is<FilterDefinition<ActividadMongo>>(f => f != null),
                It.IsAny<FindOptions<ActividadMongo, ActividadMongo>>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(asyncCursor.Object);

        mockDatabase.Setup(d => d.GetCollection<ActividadMongo>("actividades", null))
            .Returns(mockCollection.Object);
        mockClient.Setup(c => c.GetDatabase("test_db", null))
            .Returns(mockDatabase.Object);

        var repository = new MongoRepository<ActividadMongo>(mockClient.Object, "test_db", "actividades");

        // Act
        var resultado = await repository.GetByIdAsync(documentoId);
    ...
    [Fact]
    public async Task GetByIdAsync_DeberiaRetornarNull_CuandoNoExisteDocumento()
    {
        // Arrange
        var documentoId = Guid.NewGuid().ToString();
        ...
        asyncCursor.SetupSequence(x => x.MoveNextAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(true)
            .ReturnsAsync(false);
        asyncCursor.Setup(x => x.Current).Returns(new List<ActividadMongo>());
